 ...
 *
 * Usage:
 *   npx design poll                         # Block until browser event, print JSON
 *   npx design poll --timeout=600000        # Custom timeout (ms); default is long-poll friendly
 *   npx design poll --reply <id> done       # Reply "done" to event <id>
 *   npx design poll --reply <id> error "msg" # Reply with error
 */
...
import { readLiveServerInfo } from './design-paths.mjs';
...
function readServerInfo() {
  ...
  if (!record) {
    console.error('No running live server found. Start one with: npx design live');
    process.exit(1);
  }
  return record.info;
}
...
export async function pollCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: design poll [options]
...
Options:
  ...
  // Reply mode: npx design poll --reply <id> <status> [--file path] [message]
  ...
  if (replyIdx !== -1) {
    ...
    if (!id) {
      console.error('Usage: npx design poll --reply <id> <status> [--file path] [message]');
      process.exit(1);
    }
    ...
    try {
      ...
      if (err.cause?.code === 'ECONNREFUSED') {
        console.error('Live server not running. Start one with: npx design live');
      } else {
  ...
  try {
    while (true) {
      ...
      if (res.status === 401) {
        ...
        console.error('Try restarting: npx design live stop && npx design live');
        process.exit(1);
      }
