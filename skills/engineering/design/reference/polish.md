...
4. **Pull in any prior critique** (optional signal): If `{{command_prefix}}design critique` has been run on the same target, its priority issues are a useful prior for what to address first. Resolve the target to a file path or URL, then:
   ```bash
