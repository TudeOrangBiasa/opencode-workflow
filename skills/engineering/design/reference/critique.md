...
**CLI scan**:
```bash
npx design detect --json [--fast] [target]
```
...
7. **Read results from console** using `read_console_messages` with pattern `design`. The detector logs all findings with the `[design]` prefix. Do NOT scroll through the page to take screenshots of the overlays.
8. **Cleanup**: Stop the live server when done:
   ```bash
   npx design live stop
   ```
...
Once the report above is finalized, write it to `.design/critique/` so the user can refer back, and so `{{command_prefix}}design polish` can pick up the priority issues without a copy-paste.
...
4. **Append a single line to the user-visible output**, after the report and before the questions:
   ...
   > Wrote `.design/critique/<filename>`.
