 ...
 *
 * On first live run, the agent generates `.design/live/config.json`
...
import { resolveLiveConfigPath } from './design-paths.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const CONFIG_PATH = resolveLiveConfigPath({ cwd: process.cwd(), scriptsDir: __dirname });
const MARKER_OPEN_TEXT = 'design-live-start';
const MARKER_CLOSE_TEXT = 'design-live-end';
...
Reads configuration from .design/live/config.json.

Modes:
  --port PORT   Insert script tag pointing at http://localhost:PORT/live.js
  --remove      Remove the script tag (if present)
  --check       Print whether .design/live/config.json exists and its content
...
function removeTag(content, _syntax) {
  const patterns = [
    /([ \t]*)<!--\s*design-live-start\s*-->[\s\S]*?<!--\s*design-live-end\s*-->[ \t]*\n/,
    /([ \t]*)\{\/\*\s*design-live-start\s*\*\/\}[\s\S]*?\{\/\*\s*design-live-end\s*\*\/\}[ \t]*\n/,
  ];
  for (const pat of patterns) {
...
// and stash the original `content` value in a `data-design-csp-original`
...
const CSP_MARKER_ATTR = 'data-design-csp-original';
