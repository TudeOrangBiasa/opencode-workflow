...
(function () {
  ...
  if (!sessionState) {
    console.error('[design] live-browser-session.js was not loaded. Live mode cannot start safely.');
  ...
  const ICONS = {
    design: `<svg ${ICON_ATTRS}><path d="M4 20l4-1L18 9l-3-3L5 16z"/><path d="M14 7l3 3"/></svg>`,
    ...
    if (savedY != null) {
      const apply = () => {
        if (Math.abs(window.scrollY - savedY) > 0.5) {
          console.log('[design.scroll] early restore', { from: window.scrollY, to: savedY });
  ...
  function buildConfigureRow() {
    ...
    input.type = 'text';
    input.placeholder = selectedAction === 'design' ? 'describe what you want...' : 'refine further (optional)...';
    Object.assign(input.style, {
      flex: '1', minWidth: '0',
  ...
  function buildSavingRow() {
    ...
    const spinner = el('div', {
      ...
      animation: 'design-spin 0.6s linear infinite',
      flexShrink: '0',
    });
    ...
    if (!document.getElementById(PREFIX + '-keyframes')) {
      ...
      style.textContent = '@keyframes design-spin { to { transform: rotate(360deg); } }';
  ...
  // variant wrapper:
  //
  //   <div data-design-variant="1"
  //        data-design-params='[{"id":"density","kind":"steps",...}]'>
  //
  ...
  function getVisibleVariantEl() {
    ...
    const wrapper = document.querySelector('[data-design-variants="' + currentSessionId + '"]');
    if (!wrapper) return null;
    return wrapper.querySelector('[data-design-variant="' + visibleVariant + '"]');
  }

  function parseVariantParams(variantEl) {
    if (!variantEl) return [];
    const raw = variantEl.getAttribute('data-design-params');
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (err) {
      console.warn('[design] Invalid data-design-params JSON:', err.message);
      return [];
    }
  }
  ...
  function showVariantInDOM(sessionId, num) {
    const wrapper = document.querySelector('[data-design-variants="' + sessionId + '"]');
    if (!wrapper) return;
    for (const child of wrapper.children) {
      const v = child.dataset ? child.dataset.designVariant : null;
      if (!v) continue;
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        const srcWrapper = doc.querySelector('[data-design-variants="' + sessionId + '"]');
        if (!srcWrapper) {
          console.error('[design] Variant wrapper not found in source file.');
          return;
        }
        ...
        const origContent = srcWrapper.querySelector('[data-design-variant="original"] > :first-child');
        ...
        if (!liveEl) {
          console.error('[design] Could not find original element in live DOM.');
          return;
        }
        ...
        const variants = wrapper.querySelectorAll('[data-design-variant]:not([data-design-variant="original"])');
        arrivedVariants = variants.length;
        expectedVariants = parseInt(wrapper.dataset.designVariantCount || arrivedVariants);
        ...
        refreshParamsPanel();
        saveSession();
        console.log('[design] Injected ' + arrivedVariants + ' variants from source file.');
      })
      .catch(err => {
        console.error('[design] Failed to fetch source:', err);
  ...
  function readVisibleVariantFromDOM(sessionId) {
    ...
    for (const variant of variants) {
      ...
      const idx = parseInt(variant.dataset.designVariant || '0', 10);
      if (idx > 0) return idx;
    }
    return 0;
  }
  ...
  function pickVariantContent(wrapper, index) {
    if (!wrapper) return null;
    const variantDiv = wrapper.querySelector('[data-design-variant="' + index + '"]');
  ...
  function startScrollLock(sessionId, initialTargetY) {
    ...
    scrollLockTargetY = typeof initialTargetY === 'number' && isFinite(initialTargetY)
      ? initialTargetY
      : window.scrollY;
    console.log('[design.scroll] startScrollLock', { sessionId, scrollY: window.scrollY, targetY: scrollLockTargetY, initialOverride: initialTargetY });
    ...
    const correct = (why) => {
      ...
      if (Math.abs(delta) < 0.5) {
        console.log('[design.scroll] correct noop', { why, scrollY: before, targetY: scrollLockTargetY });
        return;
      }
      window.scrollTo({ top: scrollLockTargetY, left: window.scrollX, behavior: 'instant' });
      console.log('[design.scroll] corrected', { why, from: before, to: scrollLockTargetY, delta, nowAt: window.scrollY });
    };
    const schedule = (why) => {
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.target?.closest?.('[data-design-variants="' + sessionId + '"]')) {
          const childAdds = Array.from(m.addedNodes).map(n => n.nodeType === 1 ? (n.tagName + (n.dataset?.designVariant ? ('[variant=' + n.dataset.designVariant + ']') : '')) : n.nodeType).join(',');
          console.log('[design.scroll] mutation inside wrapper', { type: m.type, target: m.target?.tagName, adds: childAdds, scrollYBefore: window.scrollY, targetY: scrollLockTargetY });
          schedule('mutation-in-wrapper');
          return;
        }
        for (const n of m.addedNodes) {
          if (n.nodeType === 1 && (n.matches?.('[data-design-variants="' + sessionId + '"]') || n.querySelector?.('[data-design-variants="' + sessionId + '"]'))) {
            console.log('[design.scroll] wrapper node added', { tag: n.tagName, scrollYBefore: window.scrollY, targetY: scrollLockTargetY });
    ...
    const reanchor = (why) => {
      ...
      console.log('[design.scroll] reanchor', { why, prevTarget, newTarget: scrollLockTargetY });
    };
    ...
    window.addEventListener('scroll', () => {
      ...
      if (Math.abs(now - lastLoggedScrollY) > 5) {
        console.log('[design.scroll] scroll event', { from: lastLoggedScrollY, to: now, targetY: scrollLockTargetY });
      ...
      console.log('[design.scroll] scroll-event snap', { from: now, to: scrollLockTargetY });
      window.scrollTo({ top: scrollLockTargetY, left: window.scrollX, behavior: 'instant' });
    }, { passive: true, ...sig });
    ...
    if (Math.abs(window.scrollY - scrollLockTargetY) > 0.5) {
      window.scrollTo({ top: scrollLockTargetY, left: window.scrollX, behavior: 'instant' });
      console.log('[design.scroll] startScrollLock initial apply', { to: scrollLockTargetY });
    }
  }
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      // Only react to mutations that add nodes with data-design-variant,
      // or mutations inside the variant wrapper. Ignore our own bar/UI changes.
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.('[data-design-variants]')) { dominated = true; break; }
        for (const n of m.addedNodes) {
          if (n.nodeType !== 1) continue;
          // Direct hit: the added node itself is the wrapper or a variant.
          if (n.dataset?.designVariants || n.dataset?.designVariant) {
          ...
          if (n.querySelector?.('[data-design-variants],[data-design-variant]')) {
            dominated = true; break;
          }
        }
        if (dominated) break;
      ...
      const expected = parseInt(wrapper.dataset.designVariantCount || '0');
  ...
  function connectSSE() {
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast('No PRODUCT.md found. Variants will be brand-agnostic. Run /design teach to generate one.', 7000);
          console.log('[design] Live mode connected.');
        ...
        case 'done':
          ...
          break;
        case 'error':
          console.error('[design] Error:', msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log('[design] SSE connection lost. Retry ' + sseRetries + '/' + SSE_MAX_RETRIES + '...');
        return; // EventSource auto-reconnects
      }
      // Server is gone. Clean up gracefully.
      console.log('[design] Live server unreachable. Cleaning up UI.');
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      console.error('[design] Failed to send event:', err);
  ...
  function handleGo() {
    ...
    console.log('[design.scroll] Go pressed', { scrollY: window.scrollY, sessionId: currentSessionId });
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn('[design] capture failed, proceeding without screenshot:', err);
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          console.warn('[design] annotation upload failed:', uploadRes.status);
        }
      } catch (err) {
        console.warn('[design] annotation upload failed:', err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect) {
    ...
    try {
      ...
      console.warn('[design] shader setup failed:', err);
      canvas.remove();
      return;
    }
  ...
  function handleAccept() {
    ...
    // on or the app isn't React at all. Preserve the `data-design-variant="N"`
    ...
    setTimeout(function() {
      const wrapper = document.querySelector('[data-design-variants="' + acceptedSessionId + '"]');
      if (!wrapper) return;
      const accepted = wrapper.querySelector('[data-design-variant="' + acceptedVariant + '"]');
  ...
  function cleanup() {
    ...
    if (cleanupSessionId) {
      const wrapper = document.querySelector('[data-design-variants="' + cleanupSessionId + '"]');
    ...
    setTimeout(function() {
      ...
      const wrapper = document.querySelector('[data-design-variants="' + cleanupSessionId + '"]');
      if (!wrapper) return;
      const orig = wrapper.querySelector('[data-design-variant="original"]');
      if (orig) {
  ...
  // If a [data-design-variants] wrapper exists in the DOM, the agent wrote
  // variants before HMR fired. Pick up where we left off.
  function resumeSession() {
    const wrapper = document.querySelector('[data-design-variants]');
    if (!wrapper) { clearSession(); clearHandled(); return false; }

    const sessionId = wrapper.dataset.designVariants;

    // Don't resume if this session was already accepted/discarded
    if (isSessionHandled(sessionId)) return false;

    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.designVariantCount || '0');
    const variants = wrapper.querySelectorAll('[data-design-variant]:not([data-design-variant="original"])');
    arrivedVariants = variants.length;
    ...
    if (state === 'GENERATING' && origEl) {
      (async () => {
        try {
          ...
          console.warn('[design] shader resume failed:', err);
        }
      })();
    }
    return true;
  ...
  function detectPageTheme() {
    try {
      // Dev override: set localStorage 'design-dev-theme' to 'light' or
      // 'dark' to preview the opposite palette without actually changing the
      // page bg. Used for screenshots and theme QA.
      const override = localStorage.getItem('design-dev-theme');
  ...
  function updateGlobalBarState() {
    ...
    document.querySelectorAll('.design-overlay').forEach(o => {
      o.style.pointerEvents = pickActive ? 'none' : '';
    });
  }

  let detectReady = false; // true once detect script posts 'design-ready'
  ...
  function toggleDetect() {
    ...
    if (detectActive) {
      if (!detectScriptLoaded) {
        ...
        window.postMessage({ source: 'design-command', action: 'scan' }, '*');
      } else {
        detectPendingScan = true;
      }
    } else {
      window.postMessage({ source: 'design-command', action: 'remove' }, '*');
      detectCount = 0;
      updateGlobalBarState();
    }
  }
  ...
  function loadDetectScript() {
    ...
    s.dataset.designExtension = 'true';
    document.head.appendChild(s);
  }
  ...
  function onDetectMessage(e) {
    ...
    if (e.data.source === 'design-results') {
  ...
  function teardown() {
    ...
    state = 'IDLE';
    window.__IMPECCABLE_LIVE_INIT__ = false;
    console.log('[design] Live mode exited.');
  }

  // ---------------------------------------------------------------------------
  // Design System Panel — visualizes the project's .design/design.json sidecar
  // ---------------------------------------------------------------------------

  const DESIGN_PREFS_KEY = 'design-live-design-panel';
  ...
  let designState = {
    ...
    sidecar: null,          // .design/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>/design document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .design/design.json.</strong> Run <code>/design document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>/design document</code> also generates a <code>.design/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function cssSafe(v) {
    ...
    // .design/design.json values rendered into inline style strings.
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log('[design] Live variant mode ready. Hover over elements to pick one.');
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          console.log('[design] Resumed deferred session ' + currentSessionId + ' (post-hydration).');
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      console.log('[design] Resumed active variant session ' + currentSessionId + ' (' + arrivedVariants + '/' + expectedVariants + ' variants).');
    }
  }
