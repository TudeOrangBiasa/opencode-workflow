...
description: Use when build MCP servers with Node/TypeScript SDK — tools, resources, prompts, Zod validation, stdio vs Streamable HTTP transport. Use only when implementing or debugging MCP servers.
---

# MCP Server Patterns

Adapted from ECC's `mcp-server-patterns` skill (MIT).
...
- **Prompts**: Reusable, parameterised prompt templates the client can surface. Register with `registerPrompt()` or equivalent.
- **Transport**: stdio for local clients; Streamable HTTP is preferred for remote clients. Legacy HTTP/SSE is for backward compatibility.
