...
/**
 * Find harness skill directories that have an design skill installed.
 */
function findHarnessDirs(projectRoot) {
  ...
  for (const harness of HARNESS_DIRS) {
    ...
    if (existsSync(designDir) || existsSync(join(skillsDir, 'i-design'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
This is a pinned shortcut for \`{{command_prefix}}design ${command}\`.

Invoke {{command_prefix}}design ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
