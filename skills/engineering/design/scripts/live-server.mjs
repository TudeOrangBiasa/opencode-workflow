...
import {
  ...
  writeLiveServerInfo,
} from './design-paths.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
// PRODUCT.md / DESIGN.md live wherever load-context.mjs resolves. The generated
// DESIGN sidecar is project-local at .design/design.json, with legacy
...
function loadBrowserScripts() {
  ...
  const detectPaths = [
    ...
    path.join(process.cwd(), 'node_modules', 'design', 'cli', 'engine', 'detect-antipatterns-browser.js'),
  ];
  let detectScript = '';
...
function hasProjectContext() {
  ...
  // .design.md is auto-migrated to PRODUCT.md by load-context.mjs.
  try {
...
const VISUAL_ACTIONS = [
  'design', 'bolder', 'quieter', 'distill', 'polish', 'typeset',
...
function createRequestHandler({ detectScript, sessionPath, livePath }) {
  return (req, res) => {
    ...
    //   /design-system.json    returns both parsed DESIGN.md and .design/design.json
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      if (jsonStat) {
        try {
          ...
          response.sidecarError = 'Failed to parse .design/design.json: ' + err.message;
        }
      }
