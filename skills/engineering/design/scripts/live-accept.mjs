...
function handleAccept(id, variantNum, lines, targetFile, paramValues) {
  ...
  const hasHelperAttrs = variantText.includes('data-design-variant');
  ...
  if (cssContent) {
    replacement.push(indent + commentSyntax.open + ' design-carbonize-start ' + id + ' ' + commentSyntax.close);
    ...
    replacement.push(indent + '<style data-design-css="' + id + '">' + (isJsx ? '{`' : ''));
    ...
    if (paramValues && Object.keys(paramValues).length > 0) {
      ...
      replacement.push(indent + commentSyntax.open + ' design-param-values ' + id + ': ' + JSON.stringify(paramValues) + ' ' + commentSyntax.close);
    }
    replacement.push(indent + commentSyntax.open + ' design-carbonize-end ' + id + ' ' + commentSyntax.close);
  }

  // Keep the `@scope ([data-design-variant="N"])` selectors in the
  // carbonize CSS block working visually by re-wrapping the accepted content
  // in a data-design-variant="N" div with `display: contents` (so layout
  ...
  if (cssContent) {
    ...
    replacement.push(indent + '<div data-design-variant="' + variantNum + '" ' + styleAttr + '>');
...
function findMarkerBlock(id, lines) {
  let start = -1;
  let end = -1;
  const startPattern = 'design-variants-start ' + id;
  const endPattern = 'design-variants-end ' + id;
 ...
 * the `<div data-design-variants="ID">` outer wrapper so the picked
 ...
 * (with `data-design-variants="ID"`) and its matching `</div>`. We
...
function expandReplaceRange(block, lines, isJsx) {
  ...
  // Walk back for the wrapper `<div data-design-variants="..."` opener.
  ...
  for (let i = start - 1; i >= Math.max(0, start - 12); i--) {
    if (/data-design-variants=/.test(lines[i])) {
      let opener = i;
 ...
 *   - CSS `@scope ([data-design-variant="N"])` strings that look like the
...
function extractOriginal(lines, block) {
  ...
  const inner = extractInnerByAttr(text, 'data-design-variant="original"');
...
function extractVariant(lines, block, variantNum) {
  const text = stripStyleAndJoin(lines, block);
  const inner = extractInnerByAttr(text, 'data-design-variant="' + variantNum + '"');
 ...
 *
 * Handles three shapes of `<style data-design-css="ID" ...>`:
...
function extractCss(lines, block, id) {
  const styleAttr = 'data-design-css="' + id + '"';
  let inStyle = false;
  const content = [];
...
function findSessionFile(id, cwd) {
  const marker = 'design-variants-start ' + id;
