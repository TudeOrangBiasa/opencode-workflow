...
- **[memory-dreaming](./skills/engineering/memory-dreaming/SKILL.md)** — Consolidate agent lessons across sessions: 2-phase pipeline (find + present/merge). Manual trigger on "consolidate", "dream", "clean up lessons", "memory hygiene".
...
- **[document-writing](./skills/productivity/document-writing/SKILL.md)** — How to write documents properly. Decision tree for from-scratch vs extending existing. 10 anti-patterns from real dbl-data-management analysis (numbering corruption, styleId by-name, choppy prose, prose in SQL, etc.).
