...
**Output**: A design brief that can be handed off to {{command_prefix}}design craft, or directly to {{command_prefix}}design for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
### Surface identification (required)
Before writing the brief, identify the surface pattern. State which surface (Monitor/Operate/Compare/Configure/Learn/Decide/Explore) this feature maps to and why. This feeds into Layout Strategy (section 5) and Recommended References (section 9).
...
Once confirmed, the brief is complete. The user can now hand it to {{command_prefix}}design, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use {{command_prefix}}design craft instead, which runs this command internally.)
