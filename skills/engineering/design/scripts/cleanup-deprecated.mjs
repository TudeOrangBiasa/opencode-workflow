 ...
 *      "design" (to avoid deleting unrelated user skills).
...
const DEPRECATED_NAMES = [
  // v2.0 renames
  'frontend-design',    // renamed to design
  'teach-design',   // folded into /design teach
  // v2.1 merges
...
// "design" in their v2.x source. Used as a last-resort match
 ...
 *   1. Lock source equals "pbakaus/design" (authoritative).
 *   2. SKILL.md body contains the word "design".
...
export function isImpeccableSkill(skillDir, { skillName, lock } = {}) {
  ...
  if (skillName && lock?.skills?.[skillName]?.source === 'pbakaus/design') {
    return true;
  }
  ...
  if (/design/i.test(content)) return true;
 ...
 * drive deletion even when SKILL.md never mentions design.
...
export function removeDeprecatedSkills(projectRoot, lock) {
  ...
  for (const skillsDir of skillsDirs) {
    for (const name of targets) {
      ...
      // Regular directory -- verify it belongs to design
 ...
 * Only removes entries whose source is "pbakaus/design".
...
export function cleanSkillsLock(projectRoot) {
  ...
  for (const name of targets) {
    ...
    if (!entry) continue;
    // Only remove if it belongs to design
    if (entry.source === 'pbakaus/design') {
