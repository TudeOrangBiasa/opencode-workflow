 ...
 *
 * Usage:
 *   npx design wrap --id SESSION_ID --count N --query "hero-combined-left" [--file path]
 *
...
export async function wrapCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: design wrap [options]
...
The agent should insert variant HTML at insertLine.`);
  ...
  const wrapperLines = isJsx ? [
    indent + '<div data-design-variants="' + id + '" data-design-variant-count="' + count + '" ' + styleContents + '>',
    indent + '  ' + commentSyntax.open + ' design-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' Original ' + commentSyntax.close,
    indent + '  <div data-design-variant="original">',
    reindentOriginal('    '),
    indent + '  </div>',
    indent + '  ' + commentSyntax.open + ' Variants: insert below this line ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' design-variants-end ' + id + ' ' + commentSyntax.close,
    indent + '</div>',
  ] : [
    indent + commentSyntax.open + ' design-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '<div data-design-variants="' + id + '" data-design-variant-count="' + count + '" ' + styleContents + '>',
...
function detectStyleMode(filePath) {
  ...
  if (ext === '.astro') {
    return {
      ...
      styleTag: '<style is:inline data-design-css="SESSION_ID">',
    };
  }
  return {
    mode: 'scoped',
    styleTag: '<style data-design-css="SESSION_ID">',
  };
}

function buildCssSelectorPrefixExamples(styleMode, count) {
  if (styleMode !== 'astro-global-prefixed') return [];
  return Array.from({ length: count }, (_, i) => `[data-design-variant="${i + 1}"]`);
}
...
function buildCssAuthoring(styleMode, count) {
  ...
  if (styleMode.mode === 'astro-global-prefixed') {
    return {
      ...
      rulePattern: '[data-design-variant="N"] > .variant-class { ... }',
      selectorExamples: variantNumbers.map((n) => `[data-design-variant="${n}"] > .variant-class`),
      requirements: [
        'Use the styleTag exactly; the is:inline attribute is required for this file.',
        'Prefix every preview selector with the matching [data-design-variant="N"] selector.',
  ...
  return {
    ...
    styleTag: styleMode.styleTag,
    strategy: 'scope-rule',
    rulePattern: '@scope ([data-design-variant="N"]) { :scope > .variant-class { ... } }',
    selectorExamples: variantNumbers.map((n) => `@scope ([data-design-variant="${n}"]) { :scope > .variant-class { ... } }`),
    requirements: [
      'Use @scope blocks keyed to each [data-design-variant="N"] wrapper.',
    ...
    forbidden: [
      'Do not use global [data-design-variant="N"] selector prefixes for this styleMode.',
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes('data-design-variant')) continue;
